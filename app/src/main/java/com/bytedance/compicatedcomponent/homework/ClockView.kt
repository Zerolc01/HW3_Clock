...
class ClockView @JvmOverloads constructor(
    ...
    init {
        ...
        hoursValuesColor = DEFAULT_PRIMARY_COLOR
    ...
    private fun drawHoursValues(canvas: Canvas) {
        ...
        val paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            style = Paint.Style.FILL_AND_STROKE
            strokeWidth = 2 * DEFAULT_DEGREE_STROKE_WIDTH
            color = hoursValuesColor
            textSize = 60f
        }
        //绘制刻度数字
        val hourTextPad: Int = centerX - (resultWidth * 0.1f).toInt()
        for (i in 0..11) {
            var degree = i * 30
            canvas.drawText(
                if (i == 0) "12" else i.toString(),
                (centerX.toFloat() + hourTextPad * sin(Math.toRadians(degree.toDouble()))).toFloat() - paint.textSize/2,
                (centerY.toFloat() - hourTextPad * cos(Math.toRadians(degree.toDouble()))).toFloat() + paint.textSize/2,
                paint
            )
        }
    ...
    private fun drawTimer(canvas: Canvas){
        val timePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            style = Paint.Style.FILL_AND_STROKE
            strokeWidth = 5 * DEFAULT_DEGREE_STROKE_WIDTH
            color = Color.GRAY
            textSize = 120f
        }
        ...
        val th = if(nowHours>9) "$nowHours" else "0$nowHours"
        val ts = if(nowSeconds>9) "$nowSeconds" else "0$nowSeconds"
        val tm = if(nowMinutes>9) "$nowMinutes" else "0$nowMinutes"
        canvas.drawText("$th:$tm:$ts", centerX.toFloat() - timePaint.textSize * 2, centerY.toFloat() + timePaint.textSize * 2.5f, timePaint)
    }
    ...
    private fun drawNeedles(canvas: Canvas) {
        ...
        // 画分针
        // todo 2: 画分针
        val partMinutes = nowSeconds / 60
        drawPointer(canvas, POINTER_TYPE_MINUTES, nowMinutes + partMinutes)
